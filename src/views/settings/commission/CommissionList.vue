          ...
          <div class="card">
            <div class="card-header d-flex justify-content-end">
              <router-link class="btn btn-sm btn-success"
                           :to="{name: 'settings_commission_add'}">
            ...
            <div class="card-body">
              <table-component
                ...
                <template v-slot:action="{ row }">
                  <router-link :to="{name: 'settings_commission_edit', params: {id: row.id}}"
...
export default {
  ...
  methods: {
    loadData() {
      ...
      this.axios.get('/commissions', {params: this.queryParams})
        .then((response) => {
    ...
    deleteData(row) {
      this.$swal({
        ...
        if (result.isConfirmed) {
          this.axios.delete('/commissions/'+row.id).then(() => {
            this.loadData();
            this.$swal.fire(
              'Deleted',
