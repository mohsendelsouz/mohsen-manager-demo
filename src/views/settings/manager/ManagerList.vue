            ...
            <div class="card-body">
              <table-component
                ...
                <template v-slot:action="{ row }">
                  <router-link :to="{name: 'settings_manager_edit', params: {id: row.id}}"
...
export default {
  ...
  methods: {
    loadData() {
      ...
      this.axios.get('/managers', {params: this.queryParams})
        .then((response) => {
    ...
    deleteData(row) {
      this.$swal({
        ...
        if (result.isConfirmed) {
          this.axios.delete('/managers/'+row.id).then(() => {
            this.loadData();
            this.$swal.fire(
              'Deleted',
