<template>
  ...
  <div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0">Machine Maintenance</h1>
        </div><!-- /.col -->
      </div><!-- /.row -->
    </div><!-- /.container-fluid -->
  </div>
          ...
          <div class="card">
            <div class="card-header d-flex justify-content-end">
              <router-link class="btn btn-sm btn-success"
                           :to="{name: 'machines_maintenances_add'}">
            ...
            <div class="card-body">
              <table-component
                ...
                <template v-slot:action="{ row }">
                  <router-link :to="{name: 'machines_maintenances_edit', params: {id: row.id}}"
...
export default {
  ...
  methods: {
    loadData() {
      ...
      this.axios.get('/machines_maintenances', {params: this.queryParams})
        .then((response) => {
    ...
    deleteData(row) {
      this.$swal({
        ...
        if (result.isConfirmed) {
          this.axios.delete('/machines_maintenances/' + row.id).then(() => {
            this.loadData();
            this.$swal.fire(
              'Deleted',
              'Machine Maintenance has been deleted!',
              'success'
            )
          });
        }
